...
async function loadStudyTemplates(forceReload = false) {
  ...
  try {
    ...
    body.innerHTML = (payload.items || []).map((item) => {
      ...
      const details = `<button type="button" class="btn btn-sm btn-outline-secondary" data-template-details="${escapeHtml(item.slug || "")}">Ver estrutura</button> <button type="button" class="btn btn-sm btn-outline-primary" data-template-preview="${escapeHtml(item.slug || "")}">Ver prévia</button>`;
    ...
    body.querySelectorAll("[data-template-preview]").forEach((button) => button.addEventListener("click", () => renderStudyTemplatePreview(window.studyTemplatesBySlug?.[button.dataset.templatePreview])));
  } catch (error) {
...
function renderStudyTemplatePreview(item) {
  const panel = document.getElementById("studyTemplatePreview");
  if (!panel || !item) return;
  const section = (number, title, content, className = "") => `<section class="study-template-preview-section ${className}"><div class="study-template-preview-heading"><span>${number}</span><h3>${title}</h3></div>${content}</section>`;
  panel.innerHTML = `<div class="table-toolbar"><div><h2>Prévia HTML · ${escapeHtml(item.administradora || "Template")}</h2><p>Estrutura visual demonstrativa. Os valores abaixo serão substituídos pelo estudo real.</p></div><button type="button" class="btn btn-outline-secondary" data-close-template-preview>Fechar</button></div><article class="study-template-preview-document"><header class="study-template-preview-cover"><img src="/static/images/crediclass-logo.png" alt="Crediclass"><span>ESTUDO FINANCEIRO</span><h2>Aquisição de imóvel</h2><p>Proposta personalizada para <strong>Cliente demonstrativo</strong></p><div><span>Administradora</span><strong>${escapeHtml(item.administradora || "ITAU")}</strong></div></header>${section("01", "Estudo financeiro", "<p>Este estudo apresenta cenários comparativos com base nas informações fornecidas pelo cliente e nas condições observadas na data de emissão.</p>", "is-institutional")}${section("02", "Simulação de investimento", "<div class=\"study-template-preview-grid\"><div><small>Crédito desejado</small><strong>R$ 2.000.000,00</strong></div><div><small>Prazo</small><strong>109 meses</strong></div><div><small>Custo estimado</small><strong>R$ 4.708.694,53</strong></div></div>")}${section("03", "Simulação melhores consórcios", "<p>Grupos selecionados com base em histórico de lances, estabilidade, saúde financeira e possibilidade de lance embutido.</p>", "is-institutional")}${section("04", "Contratação", "<div class=\"study-template-preview-table\"><div><b>Grupo</b><b>Crédito</b><b>Parcela</b><b>Prazo</b></div><div><span>Grupo demonstrativo</span><span>R$ 2.000.000,00</span><span>R$ 21.500,00</span><span>109 meses</span></div></div>")}${section("05", "Estratégias de contemplação", "<div class=\"study-template-preview-grid\"><div><small>Investidor</small><strong>Lance fixo</strong></div><div><small>Conservadora</small><strong>Lance livre</strong></div><div><small>Moderada</small><strong>Lance livre</strong></div></div>")}${section("06", "Histórico de lances contemplados", "<div class=\"study-template-preview-chart\"><span>Histórico mensal dos grupos selecionados será preenchido pelo motor financeiro.</span></div>")}${section("07", "Projeção de contemplação", "<div class=\"study-template-preview-grid\"><div><small>Crédito</small><strong>R$ 2.000.000,00</strong></div><div><small>Lance total</small><strong>R$ 960.000,00</strong></div><div><small>Parcela</small><strong>R$ 21.500,00</strong></div></div>")}${section("08", "Datas limites para adesão", "<div class=\"study-template-preview-deadlines\"><span>Primeira parcela <b>06/07/2026</b></span><span>Próxima assembleia <b>17/07/2026</b></span><span>Pagamento do lance <b>24/07/2026</b></span></div>")}${section("09", "Considerações importantes", "<p>Os cenários possuem caráter informativo e ilustrativo. Não constituem garantia de contemplação, rentabilidade ou resultado futuro.</p>", "is-legal")}<footer>Crediclass · Template ${escapeHtml(item.versao || "1.0")} · Prévia estrutural</footer></article>`;
  panel.classList.remove("d-none");
  panel.querySelector("[data-close-template-preview]")?.addEventListener("click", () => panel.classList.add("d-none"));
}
...
function applyInvestorPreferences(items, selectedFlags) {
  ...
  selectedFlags.forEach((flag) => {
    ...
    const values = items.map((item) => investorPreferenceNumber(definition.value ? definition.value(item) : item[definition.field], flag === "maior_lance_embutido"));
  ...
  return items.map((item) => {
    ...
    const highlights = selectedFlags
      ...
      .map((flag) => {
        const definition = definitions[flag];
        const value = definition.value ? definition.value(item) : item[definition.field];
        return `${definition.label}: ${formatInvestorPreferenceValue(flag, value)}`;
      });
    return {
      ...item,
...
function motor360HistoricalAverages(item) {
  const capacities = item?.capacidade_contemplacoes || {};
  const formatAverage = (strategy) => {
    const value = Number(capacities[strategy]?.media_contemplacoes);
...
function motor360GroupAnchorId(groupId) {
  const normalized = String(groupId ?? "").trim().replace(/[^a-zA-Z0-9_-]+/g, "-");
  return `motor360-group-${normalized || "unknown"}`;
}

function focusMotor360Group(anchorId) {
  const target = document.getElementById(anchorId);
  if (!target) return;
  target.scrollIntoView({ behavior: "smooth", block: "start" });
  target.classList.remove("motor360-group--focus");
  void target.offsetWidth;
  target.classList.add("motor360-group--focus");
...
function renderMotor360GroupCard(item) {
  ...
  const scaledProfileCards = profiles.map((profile) => {
    ...
    return `<div class="motor360-profile-card"><strong>${escapeHtml(profileDisplayLabels[profile.id] || profile.label)}</strong><div class="motor360-profile-card-values">${values}</div></div>`;
  }).join("");
...
function renderMotor360FloatingSelectionSummary() {
  ...
  const minimizeButton = `<button type="button" class="motor360-floating-minimize" data-floating-summary-toggle aria-expanded="${investorState.floatingSummaryMinimized ? "false" : "true"}" title="${investorState.floatingSummaryMinimized ? "Expandir composição" : "Minimizar composição"}">${investorState.floatingSummaryMinimized ? "+" : "−"}</button>`;
  if (!items.length) {
...
function renderSelectedGroupComparisonColumn(item, index) {
  ...
  const groupFinancialInputs = motor360ScenarioSourceMetrics(item);
...
function selectedGroupAnalytics(item) {
  ...
  const historyFit = Math.min(1, history("moderate") / 6);
  const costRate = Number(item.taxa_total ?? item.taxa_administracao ?? 0);
...
function renderSelectedGroupsECharts(items) {
  ...
  const radarAnalytics = analytics.map((entry) => ({ ...entry, profile: (id) => ({ ...entry.profile(id), percentual_referencia: selectedGroupsPercentValue(entry.profile(id).percentual_referencia) }) }));
  init("sgProfilesChart", {
    animationDuration: 420, color: colors, legend: { bottom: 0, data: radarAnalytics.map((entry) => `Grupo ${entry.groupId}`) }, tooltip: { trigger: "item", valueFormatter: (value) => `${Number(value).toLocaleString("pt-BR")}%` }, radar: { indicator: [{ name: "Conservador", max: 100 }, { name: "Moderado", max: 100 }, { name: "Agressivo", max: 100 }, { name: "Super agressivo", max: 100 }], splitArea: { areaStyle: { color: ["#fbfcfc", "#f3f7f8"] } } }, series: [{ type: "radar", data: radarAnalytics.map((entry) => ({ name: `Grupo ${entry.groupId}`, value: ["conservative", "moderate", "aggressive", "super_aggressive"].map((id) => Number(entry.profile(id).percentual_referencia || 0)), areaStyle: { opacity: .08 }, symbolSize: 6 })) }]
  });
...
function renderSelectedGroupsExtraVisuals(items) {
  ...
  const embeddedImpact = analytics.map((entry) => {
    ...
    return { before: entry.credit, after: embeddedCredit, reduction: Math.max(0, entry.credit - embeddedCredit) };
  });
  init("sgCostBenefitChart", { animationDuration: 420, grid: { left: 76, right: 35, top: 38, bottom: 48 }, tooltip: { trigger: "item", formatter: ({ data }) => `<strong>${data.name}</strong><br/>Crédito líquido: ${formatMoney(data.value[1])}<br/>Custo total estimado: ${formatMoney(data.value[0])}<br/>Eficiência: ${formatMoney(data.value[1] / Math.max(data.value[0], 1))} de crédito por R$ 1 pago` }, xAxis: { type: "value", name: "Custo total (R$)", nameLocation: "middle", nameGap: 30, axisLabel: { formatter: (v) => formatMoney(v) } }, yAxis: { type: "value", name: "Crédito líquido (R$)", axisLabel: { formatter: (v) => formatMoney(v) } }, series: [{ type: "scatter", symbolSize: 22, data: analytics.map((entry, index) => ({ name: names[index], value: [entry.totalPaid, entry.credit], itemStyle: { color: ["#ef7a24", "#1d7188", "#5a8d5b", "#8b5e9d"][index % 4] }, label: { show: true, formatter: names[index], position: "top", color: "#27404a", fontSize: 11 } })) }] });
  init("sgRiskSpeedChart", { animationDuration: 420, grid: { left: 60, right: 35, top: 38, bottom: 48 }, tooltip: { trigger: "item", formatter: ({ data }) => `<strong>${data.name}</strong><br/>Prazo restante: ${data.value[0]} meses<br/>Comprometimento da renda: ${Number(data.value[1]).toFixed(1)}%<br/>Nota: ${data.score}/100<br/>Risco: ${data.risk}` }, xAxis: { type: "value", name: "Prazo restante (meses)", nameLocation: "middle", nameGap: 30 }, yAxis: { type: "value", name: "Renda comprometida (%)", max: 100 }, series: [{ type: "scatter", symbolSize: 24, data: analytics.map((entry, index) => ({ name: names[index], score: entry.score, risk: entry.riskLabel, value: [Number(entry.item.prazo_restante || 0), Number((entry.incomeCommitment || 0) * 100).toFixed(1)], itemStyle: { color: entry.score >= 70 ? "#1d9b59" : entry.score >= 50 ? "#ef7a24" : "#a44a52" }, label: { show: true, formatter: names[index], position: index % 2 ? "bottom" : "top", color: "#27404a", fontSize: 11 } })) }] });
}
...
function renderSelectedGroupsScoreBreakdown(items) {
  ...
  const compactTable = analytics.map((entry) => `<tr><th>Grupo ${escapeHtml(entry.groupId)}<small>${escapeHtml(entry.item.administradora || "-")}</small></th>${compactProfiles.map((profileId) => { const profile = entry.profile(profileId); const ideal = Number(profile.lance_ideal || 0); const gap = Math.max(0, ideal - entry.availableBid); const ok = gap === 0 && ideal > 0; return `<td class="${ok ? "is-ok" : "is-fail"}"><b>${ok ? "✓" : "×"}</b><span>${ideal ? formatMoney(ideal) : "Sem dado"}</span>${!ok && ideal ? `<button type="button" class="sg-info sg-compact-info" aria-label="Ver valor faltante" data-tooltip="Faltam ${formatMoney(gap)} para atingir o lance ideal. Disponível: ${formatMoney(entry.availableBid)} · Ideal: ${formatMoney(ideal)}">i</button>` : ""}</td>`; }).join("")}<td><strong>${formatMoney(entry.installment)}</strong><small>Pós ${formatMoney(entry.installmentAfter)}</small></td><td><strong>${entry.contractedCredit >= entry.desiredCredit ? "✓" : "×"}</strong><small>${formatMoney(entry.contractedCredit)}</small></td></tr>`).join("");
...
function renderSelectedGroupsFinalMatrix(items) {
  ...
  const { client, own, fgts, availableBid } = selectedGroupsClientFinancials();
  const desiredInstallment = Number(client.parcela_desejada || 0);
  const maxInstallment = Number(client.parcela_maxima || 0);
  const financialData = analytics.map((entry) => ({ desired: desiredInstallment, initial: entry.installment, after: entry.installmentAfter, income: Number(client.renda_total || client.renda || 0) }));
  init("sgFinancialCompatibilityChart", { grid: { left: 92, right: 20, top: 38, bottom: 52 }, legend: { bottom: 0 }, tooltip: { trigger: "axis", axisPointer: { type: "shadow" }, valueFormatter: (v) => formatMoney(v) }, xAxis: { type: "category", data: labels }, yAxis: { type: "value", name: "Valor (R$)", axisLabel: { formatter: (v) => formatMoney(v) } }, series: [{ name: "Parcela máxima desejada", type: "bar", data: financialData.map((d) => d.desired), itemStyle: { color: "#4f6fdc" } }, { name: "Parcela inicial", type: "bar", data: financialData.map((d) => d.initial), itemStyle: { color: "#ef7a24" } }, { name: "Parcela pós-contemplação", type: "bar", data: financialData.map((d) => d.after), itemStyle: { color: "#1d9b59" } }, { name: "Renda disponível", type: "bar", data: financialData.map((d) => d.income), itemStyle: { color: "#65717a" } }] });
  const focusProfile = investorState.selectedGroupProfile && investorState.selectedGroupProfile !== "all" ? investorState.selectedGroupProfile : "moderate";
  init("sgBidComparisonChart", { grid: { left: 92, right: 20, top: 38, bottom: 52 }, legend: { bottom: 0 }, tooltip: { trigger: "axis", axisPointer: { type: "shadow" }, valueFormatter: (v) => formatMoney(v) }, xAxis: { type: "category", data: labels }, yAxis: { type: "value", name: "Valor (R$)", axisLabel: { formatter: (v) => formatMoney(v) } }, series: [{ name: "Lance próprio", type: "bar", stack: "available", data: analytics.map((entry) => Number(own) * entry.quotaCount), itemStyle: { color: "#4f6fdc" } }, { name: "Lance FGTS", type: "bar", stack: "available", data: analytics.map((entry) => Number(fgts) * entry.quotaCount), itemStyle: { color: "#a8cf32" } }, { name: "Lance embutido", type: "bar", data: analytics.map((entry) => entry.embedded), itemStyle: { color: "#ef7a24" } }, { name: "Lance total", type: "line", data: analytics.map((entry) => entry.bid), itemStyle: { color: "#1d7188" }, lineStyle: { width: 3 } }, { name: "Lance ideal", type: "line", data: analytics.map((entry) => Number(entry.profile(focusProfile).lance_ideal || 0) * entry.quotaCount), itemStyle: { color: "#b42318" }, lineStyle: { width: 3, type: "dashed" } }] });
}
...
function financialStudySectionPreferences() {
  const defaults = { agenda: true, cliente: true, resumo: true, grupos: true, contemplacao: true, observacoes: true };
  try {
...
function formatGroupDueDate(value) {
  const text = String(value ?? "").trim();
...
function financialStudyFormatCalendarDate(date) {
...
async function financialStudyAssemblyData() {
  if (assemblyState.loaded && assemblyState.data) return assemblyState.data;
...
function financialStudyAssemblySection(data, administrators, generatedAt, loadError = "") {
  const issued = new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(generatedAt);
  if (loadError) {
    return `<div class="financial-study-agenda-state is-warning"><strong>Agenda operacional indisponível</strong><span>${escapeHtml(loadError)}</span></div>`;
  }
  const { cycles, rules } = financialStudyAssemblyAgenda(data, administrators, generatedAt);
  if (!cycles.length) {
...
async function renderFinancialStudyScreen() {
  ...
  screen.innerHTML = `<div class="financial-study-page">
    ...
    <article class="financial-study-document" aria-hidden="false">
      <header class="financial-study-cover"><div class="financial-study-cover-brand"><img src="/static/images/crediclass-logo.png" alt="Crediclass" class="financial-study-logo"><span class="financial-study-kicker">CREDICLASS</span><h2>Estudo Financeiro</h2><p>Comparativo dos grupos selecionados para apoiar uma decisão clara e auditável.</p></div><dl><div><dt>Nome do cliente</dt><dd id="financialStudyHeaderClient">${escapeHtml(clientName)}</dd></div><div><dt>Data do estudo</dt><dd id="financialStudyHeaderDate">${escapeHtml(issueDate)}</dd></div><div><dt>Número do estudo</dt><dd id="financialStudyHeaderNumber">${escapeHtml(proposalId)}</dd></div></dl></header>
      ...
      <section class="financial-study-section" data-study-content="modelo"><div class="financial-study-section-heading"><span>04</span><div><h3>Simulação melhores consórcios</h3><p>Critérios e condições dos grupos selecionados.</p></div></div><div class="financial-study-model-text"><p>Os grupos apresentados foram selecionados a partir de critérios técnicos definidos pela Crediclass, considerando indicadores históricos e condições disponíveis na data da análise.</p><ol><li>Grupos antigos e histórico de contemplações;</li><li>Histórico e estabilidade dos lances;</li><li>Saúde financeira do grupo;</li><li>Aplicabilidade de lance embutido quando disponível.</li></ol></div></section>
      <section class="financial-study-section" data-study-content="modelo"><div class="financial-study-section-heading"><span>05</span><div><h3>Como funciona o consórcio</h3></div></div><div class="financial-study-model-text"><p>O consórcio é uma modalidade de crédito planejado baseada na formação de um fundo comum entre participantes.</p><p>As contemplações podem ocorrer por sorteio e lance, conforme as regras da administradora e os dados oficiais do grupo.</p></div></section>
      <section class="financial-study-section" data-study-content="modelo"><div class="financial-study-section-heading"><span>06</span><div><h3>Estratégias de contemplação</h3></div></div><div class="financial-study-model-text"><p>As estratégias são apresentadas com base no histórico dos grupos analisados. Os percentuais e valores exibidos permanecem oficiais e não são editáveis.</p></div></section>
      <section class="financial-study-section" data-study-content="modelo"><div class="financial-study-section-heading"><span>07</span><div><h3>Considerações importantes</h3></div></div><div class="financial-study-model-text"><p>Os cenários e projeções possuem caráter informativo e ilustrativo. A decisão pela contratação é de responsabilidade do cliente.</p></div></section>
...
function renderInvestorAnalysis(result) {
  ...
  results.querySelectorAll("[data-floating-summary-toggle]").forEach((button) => button.addEventListener("click", () => {
    ...
    const summaryCard = button.closest(".motor360-floating-summary");
    summaryCard?.classList.toggle("is-minimized", investorState.floatingSummaryMinimized);
    button.textContent = investorState.floatingSummaryMinimized ? "+" : "−";
    button.title = investorState.floatingSummaryMinimized ? "Expandir composição" : "Minimizar composição";
    button.setAttribute("aria-expanded", investorState.floatingSummaryMinimized ? "false" : "true");
  }));
...
function updateClientProfileTotals() {
  ...
  const lanceParticipantes = Number(holderSummary.lance_recursos_proprios || 0);
  const lance = lanceParticipantes > 0 ? lanceParticipantes : lanceManual;
  if (lanceParticipantes > 0) {
    setMoneyInputValue("clientProfileLanceProprio", lanceParticipantes);
  }
...
function applyImportedCrmData(data, opportunityId) {
  ...
  const profile = {
    ...
    renda_titular: imported.renda_titular ?? "",
    renda_conjuge: imported.renda_conjuge ?? "",
    fgts_titular: imported.fgts_titular ?? "",
    fgts_conjuge: imported.fgts_conjuge ?? "",
    fgts: imported.fgts ?? "",
    parcela_desejada: imported.parcela_desejada ?? "",
    tipo_bem: imported.tipo_bem ?? "",
    objetivo: imported.objetivo || "",
    nome_conjuge: imported.nome_conjuge || "",
    data_nascimento_conjuge: imported.data_nascimento_conjuge || "",
...
function renderStudyDetails(item) {
  ...
  document.getElementById("studyDetailsMeta").textContent = `${item.operador || "Não informado"} - ${item.criado_em || "-"}`;
