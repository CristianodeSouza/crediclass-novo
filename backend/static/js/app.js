...
function renderSelectedGroupsExtraVisuals(items) {
  ...
  init("sgCostBenefitChart", { animationDuration: 420, tooltip: { trigger: "item", formatter: ({ data }) => `${data.name}<br/>Crédito: ${formatMoney(data.value[1])}<br/>Custo total: ${formatMoney(data.value[0])}` }, xAxis: { type: "value", name: "Custo total", axisLabel: { formatter: (v) => formatMoney(v) } }, yAxis: { type: "value", name: "Crédito", axisLabel: { formatter: (v) => formatMoney(v) } }, series: [{ type: "scatter", symbolSize: 18, data: analytics.map((entry, index) => ({ name: names[index], value: [entry.totalPaid, entry.credit], itemStyle: { color: ["#ef7a24", "#1d7188", "#5a8d5b", "#8b5e9d"][index % 4] } })) }] });
