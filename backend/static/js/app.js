...
function renderSelectedGroupsScoreBreakdown(items) {
  ...
  const compactTable = analytics.map((entry) => `<tr><th>Grupo ${escapeHtml(entry.groupId)}<small>${escapeHtml(entry.item.administradora || "-")}</small></th>${compactProfiles.map((profileId) => { const profile = entry.profile(profileId); const ok = entry.availableBid >= Number(profile.lance_ideal || 0); return `<td class="${ok ? "is-ok" : "is-fail"}"><b>${ok ? "✓" : "×"}</b><span>${profile.lance_ideal ? formatMoney(profile.lance_ideal) : "Sem dado"}</span></td>`; }).join("")}<td><strong>${formatMoney(entry.installment)}</strong><small>Pós ${formatMoney(entry.installmentAfter)}</small></td><td><strong>${entry.contractedCredit >= entry.desiredCredit ? "✓" : "×"}</strong><small>${formatMoney(entry.contractedCredit)}</small></td></tr>`).join("");
  const compactView = analytics.length > 4 ? `<div class="sg-compact-summary"><p>Visão resumida para ${analytics.length} grupos. O valor em cada perfil é o lance ideal; ✓ indica que o lance disponível atende.</p><div class="sg-compact-table-wrap"><table class="sg-compact-table"><thead><tr><th>Grupo</th>${compactProfiles.map((id) => `<th>${compactLabels[id]}</th>`).join("")}<th>Parcelas</th><th>Crédito contratado</th></tr></thead><tbody>${compactTable}</tbody></table></div><details><summary>Ver checklist detalhado por grupo</summary><div class="sg-check-grid">${analytics.map((entry) => `<div class="sg-check-column"><strong>Grupo ${escapeHtml(entry.groupId)}</strong>${checklist(entry)}</div>`).join("")}</div></details></div>` : `<div class="sg-check-grid">${analytics.map((entry) => `<div class="sg-check-column"><strong>Grupo ${escapeHtml(entry.groupId)}</strong>${checklist(entry)}</div>`).join("")}</div>`;
