...
function financialStudyGroupCard(item, assemblyData, generatedAt, assemblyError = "") {
  ...
  const scenarioColumn = (title, scenario) => `<section class="financial-study-group-scenario"><header><strong>${title}</strong></header><div><span>Crédito</span><b>${scenario.credito_contratado == null ? "Não disponível" : formatMoney(scale(scenario.credito_contratado))}</b></div><div><span>Parcela inicial</span><b>${scenario.parcela_inicial == null ? "Não calculada" : formatMoney(scale(scenario.parcela_inicial))}</b></div><div><span>Pós</span><b>${scenario.parcela_pos_contemplacao == null ? "Não calculada" : formatMoney(scale(scenario.parcela_pos_contemplacao))}</b></div><div><span>Saldo</span><b>${scenario.saldo_devedor == null ? "Não calculado" : formatMoney(scale(scenario.saldo_devedor))}</b></div></section>`;
