...
function renderFinancialStudyPdfDocument({ items, profile, preferences, clientName, generatedAt, issueDate, proposalId, highlightedGroup, highlightedStrategy, assemblyData, assemblyError }) {
  ...
  const intro = `<section class="financial-study-pdf-cover financial-study-executive${sectionClass("cliente")}" data-study-content="cliente"><div class="financial-study-pdf-cover-title">ESTUDO FINANCEIRO</div><div class="financial-study-pdf-cover-layout"><div class="financial-study-pdf-cover-main"><div class="financial-study-pdf-cover-hero"></div><div class="financial-study-pdf-client-meta">${financialStudyPdfMetaItem("Cliente", clientName)}${financialStudyPdfMetaItem("Objetivo", objective, "is-wide")}${financialStudyPdfMetaItem("Opção em destaque", `Grupo ${highlightedGroup} (${highlightedStrategy})`)}${financialStudyPdfMetaItem("Identificador", proposalId)}</div></div><aside class="financial-study-pdf-cover-notes"><div class="financial-study-pdf-cover-note is-intro"><strong>Prezado,</strong></div>${introNotes.map((note) => `<div class="financial-study-pdf-cover-note"><p>${escapeHtml(note)}</p></div>`).join("")}</aside></div></section>`;
