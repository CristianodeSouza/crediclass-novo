...
function renderSelectedGroupsAnalyticalPanel(items) {
  ...
  const ranking = ranked.map((entry, index) => `<div class="sg-ranking-row"><b>${index + 1}</b><span><strong>Grupo ${escapeHtml(entry.groupId)}</strong><small>${escapeHtml(entry.item.administradora || "-")}</small></span><em>${entry.score}/100</em><mark class="${index === 0 ? "is-best" : ""}">${index === 0 ? "Recomendado" : index === 1 ? "Alternativa" : "Comparar"}</mark></div>`).join("");
  const profileFilter = investorState.selectedGroupProfile || "all";
  const currentSort = investorState.selectedGroupSort || "score";
  const controls = `<div class="sg-controls"><label>Perfil<select data-sg-filter="profile"><option value="all" ${profileFilter === "all" ? "selected" : ""}>Todos</option><option value="conservative" ${profileFilter === "conservative" ? "selected" : ""}>Conservador</option><option value="moderate" ${profileFilter === "moderate" ? "selected" : ""}>Moderado</option><option value="aggressive" ${profileFilter === "aggressive" ? "selected" : ""}>Agressivo</option><option value="super_aggressive" ${profileFilter === "super_aggressive" ? "selected" : ""}>Super agressivo</option></select></label><label>Criterio<select data-sg-sort><option value="score" ${currentSort === "score" ? "selected" : ""}>Recomendação</option><option value="credit" ${currentSort === "credit" ? "selected" : ""}>Maior crédito</option><option value="installment" ${currentSort === "installment" ? "selected" : ""}>Menor parcela</option><option value="history" ${currentSort === "history" ? "selected" : ""}>Melhor histórico</option></select></label></div>`;
